...
interface TripDetailsProps {
  tripId?: string;
  initialTab?: 'itinerary' | 'reservations' | 'participants';
...
export const TripDetails: React.FC<TripDetailsProps> = ({ tripId, initialTab, onNavigate }) => {
  ...
  const [activeTab, setActiveTab] = useState<'itinerary' | 'reservations' | 'participants'>(initialTab || 'itinerary');
    ...
    <div className="pb-20 md:pb-8">
      ...
      <div className="sticky top-0 z-40 bg-white border-b border-gray-200 shadow-sm px-4 md:px-8">
        <div className="flex gap-6 overflow-x-auto no-scrollbar">
            {['itinerary', 'reservations', 'participants'].map((tab) => (
                <button
                    key={tab}
        ...
        {activeTab === 'participants' && (
            <div className="space-y-6 animate-in slide-in-from-bottom-2 duration-300">
                <div className="bg-blue-50 border border-blue-100 rounded-xl p-6 text-center">
                    <h3 className="text-lg font-bold text-blue-800 mb-2">Convide seus amigos</h3>
                    <p className="text-blue-600 mb-4 text-sm">Organize a viagem juntos em tempo real.</p>
                    <div className="flex justify-center -space-x-2 mb-4">
                         <div className="w-10 h-10 rounded-full bg-gray-300 border-2 border-white flex items-center justify-center text-xs font-bold text-gray-600">EU</div>
                         <div className="w-10 h-10 rounded-full bg-gray-200 border-2 border-white flex items-center justify-center text-xs text-gray-400">+</div>
                    </div>
                    <Button onClick={() => onNavigate('upgrade')}>Convidar Pessoas (Pro)</Button>
                </div>
            </div>
        )}
      </div>
