...
export const FinanceModule: React.FC<FinanceModuleProps> = ({ tripId, tripDefaultCurrency }) => {
    ...
    const loadParticipants = async () => {
        try {
            ...
            // Default splitter: all
            setSplitWith(data.map(p => p.id));
            // Default payer: first one (or ideally current user if possible, but identifying current user ID in frontend needs helper)
        ...
        <div className="bg-white shadow rounded-lg p-6 mt-6">
            <div className="flex border-b mb-6">
                <button
                    ...
                    className={`px-4 py-2 mr-2 font-medium ${activeTab === 'expenses' ? 'border-b-2 border-teal-600 text-teal-700' : 'text-gray-500 hover:text-gray-700'}`}
                >
                    Gastos
                </button>
                <button
                    onClick={() => setActiveTab('balances')}
                    className={`px-4 py-2 font-medium ${activeTab === 'balances' ? 'border-b-2 border-teal-600 text-teal-700' : 'text-gray-500 hover:text-gray-700'}`}
                >
                    Saldos & Acertos
                </button>
            </div>
                    ...
                    <div>
                        ...
                        <ul className="space-y-3">
                            {participants.map(p => {
                                ...
                                if (Math.abs(val) < 0.01) return null; // Hide zero balances? Or show as settled?
                                return (
